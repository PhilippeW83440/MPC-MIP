...
function path1d_cost(mpc::MpcPath1d, x::Variable, slack_col, slack_speed)
	cost = 0
	...
	cost += 100*sum(slack_col) # Elastic Model for Collision Avoidance
	cost += sum(slack_speed) # Elastic Model for Simplex speed
	return cost
end

function path1d_constraints(mpc::MpcPath1d, x::Variable, p, slack_col, slack_bin, slack_speed)
	...
	for k in range(1+mpc.nvars_dt, step=mpc.nvars_dt, length=mpc.T-1)
		...
		p.constraints += [18 - slack_speed[t] <= x[k+1]]
		p.constraints += [slack_speed[t] <= 18] # 0 <= slack_speed already constrained
		t += 1
...
mutable struct SimplexData
	...
	slack_speed::Vector{Variable} # Elastic Model for speed
	...
	function SimplexData(mpc)
		...
		s.slack_speed = []
		for k in 1:mpc.T
			push!(s.slack_speed, Variable(1, Positive()))
		end
		...
		cost = path1d_cost(mpc, s.x, s.slack_col, s.slack_speed)
		s.p = minimize(cost)
		path1d_constraints(mpc, s.x, s.p, s.slack_col, s.slack_bin, s.slack_speed)
...
function dump_sol(solv::SimplexData)
	...
	for (j, slack) in enumerate(solv.slack_speed)
		println("slack_speed=", slack.value)
	end
end
